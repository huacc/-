...
import WorkflowTab from './prompt/WorkflowTab';
import QualityTab from './prompt/QualityTab';
import VariableTab from './prompt/VariableTab';
import PreviewTab from './prompt/PreviewTab';
...
const StructuredPromptEditor: React.FC<StructuredPromptEditorProps> = ({ 
  ...
  const handleWorkflowChange = (newWorkflow: PromptStructure['workflow']) => {
    setData(prev => ({ ...prev, workflow: newWorkflow }));
    setInternalIsDirty(true);
  };

  const handleQualityChange = (newQualityData: PromptStructure['quality']) => {
    setData(prev => ({ ...prev, quality: newQualityData }));
    setInternalIsDirty(true);
  };

  const handleVariablesChange = (newVariables: PromptStructure['variables']) => {
    setData(prev => ({ ...prev, variables: newVariables }));
    setInternalIsDirty(true);
  };
  ...
  const renderTabContent = () => {
    switch (activeTab) {
      ...
      case 'workflow':
        return <WorkflowTab data={data.workflow} onChange={handleWorkflowChange} />;
      case 'quality':
        return <QualityTab data={data.quality} onChange={handleQualityChange} />;
      case 'variables':
        return <VariableTab data={data.variables} fullStructure={data} onChange={handleVariablesChange} />;
      case 'preview':
